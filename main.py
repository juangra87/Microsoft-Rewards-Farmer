...
def main():
    ...
    while 1:
        accountIsDone = [False] * len(loadedAccounts)
        while not all(result is True for result in accountIsDone):
            for i, currentAccount in enumerate(loadedAccounts):
                try:
                    if not accountIsDone[i]:
